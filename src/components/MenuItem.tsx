...
const MenuItem: React.FC<{
  ...
  resInfo: Info;
}> = ({ item, cartItems, addItem, resInfo }) => {

  const {removeItem} = useContext(CartContext)!;

  const notify = () => toast.success(`Added ${item?.card?.info?.name} to cart!`, {
    duration: 2000,
    position: "top-center"
  });
          ...
          <div className="flex sm:flex-col gap-4 justify-center items-center space-x-2">
            <div className="flex items-center bg-gray-100 rounded-full">
              <button
                ...
                onClick={() => removeItem(Number(item?.card?.info?.id))}
              ...
              <span className="mx-2 w-8 text-center">
                {cartItems.find(
                  (i) => Number(i.id) === Number(item?.card?.info?.id)
                )?.quantity || 0}
              </span>
              <button
                className="p-2 rounded-full hover:bg-gray-200 transition-colors duration-200"
